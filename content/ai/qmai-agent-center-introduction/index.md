...
它不是业务系统，也不是普通文档仓库，而是一套给 Codex、Cursor、Copilot 等 AI 编码工具使用的规则与流程中枢。

它解决的核心问题是：让 AI 进入任意 Qmai 国际化前端项目后，能够自动识别项目类型、选择正确工作流，并按团队规范完成需求分析、代码修改、验证和交付。

简单说，它让 AI 从“会写代码”变成“懂企迈国际化前端工作流，并按团队规则做事”。

## 先看哪里

如果只是想快速查文档，可以直接进入 [Qmai Agent Center 规范库]({{< relref "/agent-center/" >}}) 或 [Qmai Agent Center 专题]({{< relref "/series/qmai-agent-center/" >}})。

| 你想做什么 | 推荐入口 |
| --- | --- |
| 先理解这套规则中心的定位 | 当前这篇总览 |
| 看全部规范文档 | [Qmai Agent Center 规范库]({{< relref "/agent-center/" >}}) |
| 按专题连续阅读 | [Qmai Agent Center 专题]({{< relref "/series/qmai-agent-center/" >}}) |
| 看项目大致目录结构和组件库 | [Qmai 国际前端项目接入速查]({{< relref "/agent-center/project-onboarding/" >}}) |
| 看 AI 做事的底线 | [Qmai 全局约束]({{< relref "/agent-center/global-constraints/" >}}) |
| 判断当前仓库是什么项目 | [Qmai 项目自动识别]({{< relref "/agent-center/project-detection/" >}}) |
| 判断任务应该走哪个 playbook | [Qmai 工作流路由]({{< relref "/agent-center/workflow-router/" >}}) |
| 看所有 playbook 和仓库职责 | [Qmai Playbook 索引]({{< relref "/agent-center/playbook-index/" >}}) |
| 从需求分析到交付 | [Qmai 国际化交付工作流]({{< relref "/agent-center/international-delivery-workflow/" >}}) |
| 看后台 UI、组件、主题规范 | [Qmai International 后台设计技术规范]({{< relref "/agent-center/international-design-technical-spec/" >}}) |

## 为什么需要它

企迈国际化前端不是单一项目，而是一组多仓库协作的系统，包含 Vue2 后台子应用、qiankun 基座、共享组件库、公共业务页、打印能力、UI 规范和预发布流程。

如果没有统一规则，AI 处理需求时容易出现这些问题：
...
| 层级 | 文档 | 解决什么问题 |
| --- | --- | --- |
| 总览层 | [Qmai Agent Center 规范库]({{< relref "/agent-center/" >}})、[Qmai Agent Center 专题]({{< relref "/series/qmai-agent-center/" >}}) | 把分散规范集中到一个入口 |
| 规则层 | [全局约束]({{< relref "/agent-center/global-constraints/" >}})、[项目自动识别]({{< relref "/agent-center/project-detection/" >}})、[工作流路由]({{< relref "/agent-center/workflow-router/" >}}) | 约束 AI 行为，判断项目类型和任务路线 |
| 接入层 | [项目接入速查]({{< relref "/agent-center/project-onboarding/" >}})、[Playbook 索引]({{< relref "/agent-center/playbook-index/" >}}) | 查项目结构、组件库、仓库职责和 playbook 清单 |
| 执行层 | [交付工作流]({{< relref "/agent-center/international-delivery-workflow/" >}})、[后台设计技术规范]({{< relref "/agent-center/international-design-technical-spec/" >}}) | 指导真实需求交付和 UI/组件落地 |
...
AI 进入任意前端项目后，会先根据当前项目自身的信息判断，而不是依赖固定本机路径。
...
- 提升 AI 交付稳定性：不再只靠临时提示词，而是按统一流程识别项目、选择工作流、检查约束。
- 降低跨仓协作风险：qiankun、共享组件、打印、公共页面等跨仓场景，会先判断上下游影响。
- 统一团队规范：分支、发布、接口、UI、i18n、RTL、安全等规范集中沉淀。
- 沉淀团队经验：把过去依赖个人经验判断的流程变成可维护、可复用、可迭代的规则资产。
- 方便持续扩展：后续新增仓库、新流程、新 UI 规范时，只需要扩展对应规则和 playbook。
