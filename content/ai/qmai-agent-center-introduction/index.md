...
- [Qmai 全局约束]({{< relref "/agent-center/global-constraints/" >}})：修改流程、最小闭环、安全、Git、发布、Feishu 和接口规则
- [Qmai 项目自动识别]({{< relref "/agent-center/project-detection/" >}})：不依赖本机路径，根据项目自身信号判断是否启用 Qmai 规则
- [Qmai 工作流路由]({{< relref "/agent-center/workflow-router/" >}})：作为唯一任务路由真源，决定任务进入哪个最小 playbook
- [Qmai Playbook 索引]({{< relref "/agent-center/playbook-index/" >}})：列出各类 playbook 的职责边界和仓库角色
- [Qmai 国际前端项目接入速查]({{< relref "/agent-center/project-onboarding/" >}})：补充仓库分工、目录结构、接口分工、样式入口和组件库依赖关系
- [Qmai 国际化交付工作流]({{< relref "/agent-center/international-delivery-workflow/" >}})：覆盖需求分析、实现、验证和交付说明的完整闭环
- [Qmai International 后台设计技术规范]({{< relref "/agent-center/international-design-technical-spec/" >}})：包含页面骨架、设计 token、组件规则和可视化 UI 样式模板

这意味着它已经具备作为团队 AI 工作流规则中心继续扩展的基础。

如果想集中查看这些文档，可以直接进入 [Qmai Agent Center 规范库]({{< relref "/agent-center/" >}})。其中 UI 规范页已经把组件主题、列表页、配置页和工作台画布做成了可视化模板，打开页面即可看到样式效果。
