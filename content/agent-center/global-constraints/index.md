...
用户输入出现 Feishu 链接时，第一步应读取链接内容，不要反复询问是否需要查看。

| 链接类型 | 读取顺序 |
| --- | --- |
| Feishu Project / story / bug / task | 优先飞书项目 MCP；当前会话未注入但本机配置可用时，先用 shell 手动调用对应 MCP 包；再尝试 `lark-cli` 或其他 CLI；最后使用 Codex Chrome 插件 |
| Feishu wiki / docx / docs | 优先飞书文档 / Wiki MCP；再尝试 `lark-cli wiki/docs/drive`；最后使用 Codex Chrome 插件 |
